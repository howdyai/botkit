            ...
            self.prompt(
                {
                    ...
                    name: 'studio_token',
                    message: chalk.bold(
                        '(Optional) Please enter your Botkit Studio token. Get one from https://studio.botkit.ai\n> '
                    )
                },
                function(result) {
...
function buildBotKit(i, bot_vars, cb) {
    ...
    if (fs.existsSync(bot_vars.name)) {
        self.log(
            'A bot called ' +
                bot_vars.name +
                ' already exist in this directory. Please try again with a different name.'
        );
        cb();
    } else {
