...
module.exports = function(controller) {
    ...
    function genConfig(bot) {
        ...
        // Map new CMS parameters to legacy Botkit Studio field names.
        if (bot.config && bot.config.BOTKIT_CMS_URI) {
            config.studio_command_uri = bot.config.BOTKIT_CMS_URI;
        }

        if (bot.config && bot.config.CMS_KEY) {
            config.studio_token = bot.config.CMS_KEY;
        }
    ...
    controller.studio.testTrigger = function(bot, input_text, user) {
        return new Promise(function(resolve, reject) {
            controller.studio.evaluateTrigger(bot, input_text, user).then(function(command) {
                if (command !== {} && command.id) {
                    resolve(true);
