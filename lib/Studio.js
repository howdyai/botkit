...
module.exports = function(controller) {
                        ...
                        controller.studio
                            ...
                            .then(function(convo) {
                                convo.on('end', function(convo) {
                                    runHooks(
                                        after_hooks[command.command] ?
                                            after_hooks[command.command].slice()
                                            : [],
                                        convo,
                                        function(convo) {
                                            controller.trigger('remote_command_end', [
                                                bot,
                                                context,
                                                command,
                                                convo
                                ...
                                runHooks(
                                    before_hooks[command.command] ?
                                        before_hooks[command.command].slice()
                                        : [],
                                    convo,
                                    function(convo) {
                                        resolve(convo);
    ...
    controller.studio.runTrigger = function(bot, input_text, user, channel, original_message) {
        ...
        return new Promise(function(resolve, reject) {
            controller.studio
                .evaluateTrigger(bot, input_text, user)
                .then(function(command) {
                    if (command !== {} && command.id) {
                        controller.trigger('command_triggered', [bot, context, command]);
        ...
        function makeHandler(options, field) {
            ...
            return {
                ...
                callback: function(response, convo) {
                    ...
                    if (
                        field.key &&
                        answer_hooks[command.command] &&
                        answer_hooks[command.command][field.key]
                    ) {
                                    ...
                                    controller.studio
                                        ...
                                        .then(function(new_convo) {
                                            convo.context.transition_to =
                                                new_convo.context.script_name || null;
                                            convo.context.transition_to_id =
                                                new_convo.context.script_id || null;
                                            convo.stop('transitioning to ' + script);

                                            // copy any question responses
                                            for (var key in convo.responses) {
                                                new_convo.responses[key] = convo.responses[key];
                                            }

                                            // copy old variables into new conversation
                                            for (var key in convo.vars) {
                                                new_convo.setVar(key, convo.vars[key]);
                                            }
                                            new_convo.context.transition_from =
                                                convo.context.script_name || null;
                                            new_convo.context.transition_from_id =
                                                convo.context.script_id || null;
                    ...
                    for (var m = 0; m < command.script[t].script.length; m++) {
                        if (command.script[t].script[m].conditional) {
                            ...
                            if (bot.type == 'ciscospark') {
                                if (
                                    command.script[t].script[m].platforms &&
                                    command.script[t].script[m].platforms.ciscospark
                                ) {
                                    // attach files.
                                    if (command.script[t].script[m].platforms.ciscospark.files) {
                                        ...
                                        for (
                                            var f = 0;
                                            f <
                                            command.script[t].script[m].platforms.ciscospark.files
                                                .length;
                                            f++
                                        ) {
                                            message.files.push(
                            ...
                            if (bot.type == 'web' || bot.type == 'socket') {
                                if (
                                    command.script[t].script[m].platforms &&
                                    command.script[t].script[m].platforms.web
                                ) {
                                    // attach files.
                                    if (command.script[t].script[m].platforms.web.files) {
                                        ...
                                        for (
                                            var f = 0;
                                            f <
                                            command.script[t].script[m].platforms.web.files.length;
                                            f++
                                        ) {
                                            // determine if this is an image or any other type of file.
                                            command.script[t].script[m].platforms.web.files[
                                                f
                                            ].image =
                                                command.script[t].script[m].platforms.web.files[
                                                    f
                                            ...
                                            message.files.push(
                                                command.script[t].script[m].platforms.web.files[f]
                                            );
                                        }
                                    }
                                }
                            ...
                            if (bot.type == 'teams') {
                                if (
                                    command.script[t].script[m].platforms &&
                                    command.script[t].script[m].platforms.teams
                                ) {
                                    // create attachments in the Botkit message

                                    if (
                                        command.script[t].script[m].platforms &&
                                        command.script[t].script[m].platforms.teams.attachmentLayout
                                    ) {
                                    ...
                                    if (command.script[t].script[m].platforms.teams.attachments) {
                                        ...
                                        for (
                                            var a = 0;
                                            a <
                                            command.script[t].script[m].platforms.teams.attachments
                                                .length;
                                            a++
                                        ) {
                                            var data =
                                            ...
                                            if (data.type == 'o365') {
                                                attachment.contentType =
                                                    'application/vnd.microsoft.card.O365Connector'; // + data.type,
                            ...
                            if (command.script[t].script[m].attachments) {
                                ...
                                for (var a = 0; a < message.attachments.length; a++) {
                                    message.attachments[a].mrkdwn_in = [
                                        'text',
                                        'pretext',
                                        'fields'
                                    ];
                            ...
                            if (command.script[t].script[m].fb_attachment) {
                                ...
                                if (message.attachment.payload.elements) {
                                    for (
                                        var e = 0;
                                        e < message.attachment.payload.elements.length;
                                        e++
                                    ) {
                                        if (
                                            !message.attachment.payload.elements[e].buttons ||
                                            !message.attachment.payload.elements[e].buttons.length
                                        ) {
                            ...
                            if (command.script[t].script[m].action) {
                                ...
                                if (message.action == 'execute_script') {
                                    var options = command.script[t].script[m];
                            ...
                            if (command.script[t].script[m].meta) {
                                for (var a = 0; a < command.script[t].script[m].meta.length; a++) {
                                    message[command.script[t].script[m].meta[a].key] =
                                        command.script[t].script[m].meta[a].value;
                                }
                            }
                                ...
                                if (!default_found) {
                                    handlers.push({
                                        ...
                                        callback: function(r, c) {
                                            runHooks(
                                                answer_hooks[command.command] ?
                                                    answer_hooks[command.command].slice()
                                                    : [],
                                                convo,
                                                function(convo) {
                                                    c.next();
