    ...
    controller.studio.compileScript = function(bot, message, command_name, topics, vars) {
        ...
        return new Promise(function(resolve, reject) {
            bot.createConversation(message, function(err, convo) {
                ...
                convo.context.script_name = command_name;
                ...
                for (var t = 0; t < topics.length; t++) {
                    ...
                    for (var m = 0; m < topics[t].script.length; m++) {
                        ...
                        if (topics[t].script[m].conditional) {

                            convo.addConditional({
                                conditional: topics[t].script[m].conditional
                            }, topic);
                        } else {
                            ...
                            if (bot.type == 'web' || bot.type == 'socket') {
                                if (topics[t].script[m].platforms && topics[t].script[m].platforms.web) {
                                    // attach files.
                                    if (topics[t].script[m].platforms.web.files) {
                                        message.files = [];
                                        for (var f = 0; f < topics[t].script[m].platforms.web.files.length; f++) {

                                            // determine if this is an image or any other type of file.
                                            topics[t].script[m].platforms.web.files[f].image =
                                              (topics[t].script[m].platforms.web.files[f].url.match(/\.(jpeg|jpg|gif|png)$/i) != null);

                                            message.files.push(topics[t].script[m].platforms.web.files[f]);
                                        }
                                    }
                                }
                            }
