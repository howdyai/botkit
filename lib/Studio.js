    ...
    controller.studio.compileScript = function(bot, message, command) {
        ...
        return new Promise(function(resolve, reject) {
            bot.createConversation(message, function(err, convo) {
                ...
                // make the script name available
                convo.context.script_name = command.command;
                convo.context.script_id = command._id;
                ...
                if (command.vars && command.vars.length) {
                    for (var v = 0; v < command.vars.length; v++) {
                        if (command.vars[v].value) {

                            // set the key/value as a mustache variable
                            // accessible as {{vars.name}} in the templates
                            convo.setVar(command.vars[v].name, command.vars[v].value);

                            // also add this as an "answer" to a question
                            // thus making it available at {{responses.name}} and
                            // convo.extractResponse(name);
                            convo.responses[command.vars[v].name] = {
                                question: command.vars[v].name,
                                text: command.vars[v].value,
                            };
                        }
                    }
                }

                for (var t = 0; t < command.topics.length; t++) {
                    var topic = command.topics[t].topic;
                    for (var m = 0; m < command.topics[t].script.length; m++) {

                        if (command.topics[t].script[m].conditional) {

                            convo.addConditional({
                                conditional: command.topics[t].script[m].conditional
                            }, topic);
                        } else {

                            var message = {};

                            if (command.topics[t].script[m].text) {
                                message.text = command.topics[t].script[m].text;
                            }

                            // handle platform specific fields
                            if (bot.type == 'ciscospark') {
                                if (command.topics[t].script[m].platforms && command.topics[t].script[m].platforms.ciscospark) {
                                    // attach files.
                                    if (command.topics[t].script[m].platforms.ciscospark.files) {
                                        message.files = [];
                                        for (var f = 0; f < command.topics[t].script[m].platforms.ciscospark.files.length; f++) {
                                            message.files.push(command.topics[t].script[m].platforms.ciscospark.files[f].url);
                                        }
                                    }
                                }
                            }
                            ...
                            if (bot.type == 'web' || bot.type == 'socket') {
                                if (command.topics[t].script[m].platforms && command.topics[t].script[m].platforms.web) {
                                    // attach files.
                                    if (command.topics[t].script[m].platforms.web.files) {
                                        message.files = [];
                                        for (var f = 0; f < command.topics[t].script[m].platforms.web.files.length; f++) {

                                            // determine if this is an image or any other type of file.
                                            command.topics[t].script[m].platforms.web.files[f].image =
                                              (command.topics[t].script[m].platforms.web.files[f].url.match(/\.(jpeg|jpg|gif|png)$/i) != null);

                                            message.files.push(command.topics[t].script[m].platforms.web.files[f]);
                                        }
                                    }
                                }
                            }
                            ...
                            if (bot.type == 'teams') {
                                if (command.topics[t].script[m].platforms && command.topics[t].script[m].platforms.teams) {
                                    // create attachments in the Botkit message

                                    if (command.topics[t].script[m].platforms && command.topics[t].script[m].platforms.teams.attachmentLayout) {
                                        message.attachmentLayout = command.topics[t].script[m].platforms && command.topics[t].script[m].platforms.teams.attachmentLayout;
                                    }

                                    if (command.topics[t].script[m].platforms.teams.attachments) {
                                        message.attachments = [];
                                        for (var a = 0; a < command.topics[t].script[m].platforms.teams.attachments.length; a++) {
                                            var data = command.topics[t].script[m].platforms.teams.attachments[a];
                                            var attachment = {};
                                            if (data.type == 'o365') {
                            ...
                            if (command.topics[t].script[m].attachments) {
                                message.attachments = command.topics[t].script[m].attachments;
                            ...
                            if (command.topics[t].script[m].fb_attachment) {
                                var attachment = command.topics[t].script[m].fb_attachment;
                            ...
                            if (command.topics[t].script[m].quick_replies) {
                                var options = command.topics[t].script[m].quick_replies;
                            ...
                            if (command.topics[t].script[m].collect) {

                                var options = command.topics[t].script[m].collect.options || [];
                                if (options.length > 0) {
                                    for (var o = 0; o < options.length; o++) {
                            ...
                            if (command.topics[t].script[m].action) {
                                message.action = command.topics[t].script[m].action;
                                // console.log('message.action: ', message.action);
                                // console.log('message: ', message);
                                // console.log('command.topics[t].script[m]: ', command.topics[t].script[m]);


                                if (message.action == 'execute_script') {
                                    var options = command.topics[t].script[m];
                            ...
                            // handle meta data
                            if (command.topics[t].script[m].meta) {
                                for (var a = 0; a < command.topics[t].script[m].meta.length; a++) {
                                    message[command.topics[t].script[m].meta[a].key] = command.topics[t].script[m].meta[a].value;
                                }
                            }
                            ...
                            if (command.topics[t].script[m].collect) {
                                ...
                                var handlers = [];
                                var options = command.topics[t].script[m].collect.options || [];
                                if (command.topics[t].script[m].collect.key) {
                                    capture_options.key = command.topics[t].script[m].collect.key;
                                }

                                if (command.topics[t].script[m].collect.multiple) {
