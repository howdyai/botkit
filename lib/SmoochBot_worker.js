...
var worker = function(botkit, config) {
    var api = new SmoochCore({
        keyId: botkit.config.smooch_app_key,
        secret: botkit.config.smooch_app_secret,
        scope: 'app'
    });
    ...
    bot.send = function(message, cb) {
        ...
        bot.api.appUsers.sendMessage(message.user, message).then(function(response) {
            if (cb) {
                cb(null, response);
            }
        }).catch(function(e) {
