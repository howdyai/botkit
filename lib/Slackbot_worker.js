...
module.exports = function(botkit, config) {
    ...
    bot.startRTM = function(cb) {
        bot.api.rtm.start({
            ...
            bot.channels = {};
            for(var channel_idx in res.channels) {
                var channel = res.channels[channel_idx];
            ...
            bot.rtm.on('open',function() {
                ...
                /**
                 * Update our cached list of channels
                 */
                bot.rtm.on('channel_joined', function(data, flags) {
                    var event = JSON.parse(data);
                    bot.channels[event.channel.id] = event.channel;
                });
                bot.rtm.on('channel_left', function(data, flags) {
                    var event = JSON.parse(data);
                    delete bot.channels[event.channel.id]
                });
