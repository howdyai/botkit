...
module.exports = function(botkit, config) {
        ...
        bot.api.rtm.start({
            ...
            bot.rtm.on('open', function() {
                ...
                bot.rtm.on('message', function(data, flags) {
                     ...
                     */
                    if (message != null && botkit.config.eventsApi !== true) {
    ...
    bot.identifyBot = function(cb) {
        var data;
        if (bot.identity && bot.identity.id !== null) {
            data = {
    ...
    bot.send = function(message, cb) {
        ...
        /**
         * If Events API is configured, or message has options that are not supported
         * by the RTM API we use the web API to send messages.
         */
        if (message.attachments || message.icon_emoji ||
            message.username || message.icon_url || bot.botkit.config.eventsApi) {
                console.log('========= Message sent with web API!')
