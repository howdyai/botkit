...
module.exports = function(botkit, config) {
    ...
    bot.startRTM = function(cb) {
        ...
        bot.api.rtm.start({
            ...
            bot.rtm.on('pong', function(obj) {
                lastPong = Date.now();
                botkit.debug('** BOT ID:', bot.identity.name, 'PONG received');
            });
            ...
            bot.rtm.on('open', function() {
                ...
                var pinger = function() {
                    ...
                    botkit.debug('** BOT ID:', bot.identity.name, 'PING sent');
                ...
                bot.rtm.on('message', function(data, flags) {
                    ...
                    try {
                        ...
                        console.log('** BOT ID:', bot.identity.name, 'RECEIVED BAD JSON FROM SLACK');
                    }
                    /**
            ...
            bot.rtm.on('error', function(err) {
                botkit.log.error('** BOT ID:', bot.identity.name, 'RTM websocket error!', err);
                if (pingTimeoutId) {
                    clearTimeout(pingTimeoutId);
                }
                botkit.trigger('rtm_close', [bot, err]);
    ...
    bot.send = function(message, cb) {
        botkit.debug('** BOT ID:', bot.identity.name, 'SAY', message);
