...
module.exports = function(botkit, config) {
    ...
    bot.sendWebhook = function(options, cb) {
        ...
        botkit.middleware.send.run(bot, options, function(err, bot, options) {
            request
                .post(bot.config.incoming_webhook.url, function(err, res, body) {
                    if (err) {
    ...
    bot.send = function(message, cb) {
        ...
        if (
            ...
            if (!bot.rtm) throw new Error('Cannot use the RTM API to send messages.');
    ...
    if (bot.config.incoming_webhook) bot.configureIncomingWebhook(config.incoming_webhook);

    if (bot.config.bot) bot.configureRTM(config.bot);
