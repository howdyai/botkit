...
function Slackbot(configuration) {
    ...
    slack_botkit.configureSlackApp = function(slack_app_config, cb) {
        ...
        if (
            !slack_app_config ||
            !slack_app_config.clientId ||
            !slack_app_config.clientSecret ||
            !slack_app_config.scopes
        ) {
            ...
            if (slack_app_config.redirectUri)
                slack_botkit.config.redirectUri = slack_app_config.redirectUri;
    ...
    slack_botkit.createHomepageEndpoint = function(webserver) {
        slack_botkit.log(
            '** Serving app landing page at : http://' +
    ...
    slack_botkit.handleWebhookPayload = function(req, res) {
        ...
        if (
            ...
            payload.token !== slack_botkit.config.clientVerificationToken
        ) {
    ...
    slack_botkit.middleware.categorize.use(function(bot, message, next) {
        var mentionSyntax =
            '<@' + bot.identity.id + '(\\|' + bot.identity.name.replace('.', '\\.') + ')?>';
    ...
    slack_botkit.middleware.format.use(function formatForSlack(
        bot,
        message,
        platform_message,
        next
        ...
        platform_message.attachments = message.attachments ?
            ...
            : null;
        platform_message.unfurl_links =
            typeof message.unfurl_links !== 'undefined' ? message.unfurl_links : null;
        platform_message.unfurl_media =
            typeof message.unfurl_media !== 'undefined' ? message.unfurl_media : null;
    ...
    slack_botkit.getAuthorizeURL = function(team_id, redirect_params) {
        ...
        var api_root = slack_botkit.config.api_root ?
            slack_botkit.config.api_root
            : 'https://slack.com';
    ...
    slack_botkit.createOauthEndpoints = function(webserver, callback) {
        ...
        slack_botkit.log(
            '** Serving oauth return endpoint: http://' +
                slack_botkit.config.hostname +
                ':' +
                slack_botkit.config.port +
                '/oauth'
                        ...
                        if (err) {
                            ...
                            slack_botkit.findTeamById(identity.team_id, function(err, team) {
                                ...
                                if (auth.incoming_webhook) {
                                    ...
                                    slack_botkit.trigger('create_incoming_webhook', [
                                        bot,
                                        team.incoming_webhook
                                    ]);
                                }
                                ...
                                slack_botkit.saveTeam(team, function(err, id) {
                                    if (err) {
                                        slack_botkit.log.error(
                                            'An error occurred while saving a team: ',
                                            err
                                        );
                                        if (callback) {
                                            callback(err, req, res);
                                        ...
                                        slack_botkit.storage.users.get(identity.user_id, function(
                                            err,
                                            user
                                        ) {
                                            isnew = false;
                                            ...
                                            slack_botkit.storage.users.save(user, function(
                                                err,
                                                id
                                            ) {
                                                if (err) {
                                                    slack_botkit.log.error(
                                                        'An error occurred while saving a user: ',
                                                        err
                                                    );
                                                    if (callback) {
                                                        callback(err, req, res);
                                                    ...
                                                    if (isnew) {
                                                        slack_botkit.trigger('create_user', [
                                                            bot,
                                                            user,
                                                            redirect_params
                                                        ]);
                                                    } else {
                                                        slack_botkit.trigger('update_user', [
                                                            bot,
                                                            user,
                                                            redirect_params
                                                        ]);
