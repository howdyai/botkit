        ...
        slack_botkit.findAppropriateTeam(payload, function(err, team) {
            if (err) {
                ...
                if (payload.type === 'event_callback') {
                    ...
                    bot.identity = {
                        id: team.bot.user_id,
                        name: team.bot.name || bot.config.bot.name
                    };
                }
            }
                                ...
                                slack_botkit.saveTeam(team, function(err, id) {
                                    if (err) {
                                        ...
                                        if (team.bot) {
                                            ...
                                            // to capture its name
                                            async.retry({ times: 5, interval: 500 }, retryNext => {
                                                auth_test({
                                                    token: team.bot.token
                                                }, (err, response) => {
                                                    if (err) {
                                                        return retryNext(err);
                                                    }

                                                    if (!response.user) {
                                                        slack_botkit.log.info("auth.test: Bot name not returned from api");
                                                        return retryNext(new Error("API did not return name"));
                                                    }

                                                    retryNext(null, response);
                                                });
                                            }, function (err, auth_data) {
                                                if (err) {
                                                    slack_botkit.log.error('An error occurred while saving bot name: ', err);
                                                } else {
