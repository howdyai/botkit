    ...
    slack_botkit.createWebhookEndpoints = function(webserver, authenticationTokens) {
        ...
        webserver.post('/slack/receive', function(req, res) {
            ...
            if (req.body.type === 'event_callback') {
                ...
                // Left this sloppy because I do not want to remove the event object
                // from its wrapper, but want to build the message object to be the
                // same as the RTM message objects 👍
                var message = req.body;
                message.events_api = true
                for (var key in req.body) {
                    if (key === 'event' && message.event !== null && typeof message.event === 'object') {
                        for (var value in message.event) {
                            message[value] = req.body.event[value]
                        }
                    } else {
                ...
                message.channel = message.event.channel;
                // console.log('==================MESSAGE AFTER FOR IN LOOP:\n', message)
                slack_botkit.findTeamById(message.team_id, function(err, team) {
                    if (err || !team) {
                        ...
                        if (bot.identity.id === null ) {
                            bot.api.auth.test({}, function(err, data) {
                                if (err) {
                                    console.log('+++++++++++Error in slack api auth test:', err)
                                }
                                ...
                                if (req.body.event.type === 'message') {
                                  // console.log('***************** botkit.receieveMessage from webhook')
                                    ...
                                    slack_botkit.trigger(message.event.type, [bot, message]);
                                }
