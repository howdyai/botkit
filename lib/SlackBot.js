...
function Slackbot(configuration) {
    ...
    slack_botkit.configureSlackApp = function(slack_app_config, cb) {
        ...
        if (!slack_app_config || !slack_app_config.clientId ||
            ...
            if (typeof(slack_app_config.user_scopes) == 'string') {
                slack_botkit.config.user_scopes = slack_app_config.user_scopes.split(/\,/);
            } else {
                slack_botkit.config.user_scopes = slack_app_config.user_scopes;
            }
            if (cb) cb(null);
        }
