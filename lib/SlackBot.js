...
function Slackbot(configuration) {
    ...
    slack_botkit.createOauthEndpoints = function(webserver, callback) {
        ...
        webserver.get('/login', function(req, res) {
            res.redirect(slack_botkit.getAuthorizeURL(undefined, ['identity.basic']));
        });
        ...
        webserver.get('/oauth', function(req, res) {
            ...
            oauth_access(opts, function(err, auth) {
                ...
                if (err) {
                    ...
                    var fullScopes = ['bot', 'channels:write', 'pins:read', 'channels:read', 'commands', 'users:read', 'team:read'];

                    for (var s in fullScopes) {
                        if (scopes.indexOf(fullScopes[s]) == -1) {
                            console.log("missing scope");
                            console.log("----------REDIRECT_-------");
                            console.log(fullScopes[s]);
                            res.redirect(slack_botkit.getAuthorizeURL(undefined, fullScopes));
                            res.end;
                            return;
                        }
                    }
                                        ...
                                        slack_botkit.storage.users.get(identity.user_id, function(err, user) {
                                            ...
                                            slack_botkit.storage.users.save(user, function(err, id) {
                                                ...
                                                if (err) {
                                                    ...
                                                    if (callback) {
                                                        callback(null, req, res, auth, state);
                                                    } else {
                                                        res.redirect('/');
                                                    }
                                                }
