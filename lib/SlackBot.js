...
function Slackbot(configuration) {
    ...
    slack_botkit.createWebhookEndpoints = function(webserver, authenticationTokens) {
        ...
        webserver.post('/slack/receive', function(req, res) {
          ...
          // console.log('=============== Received Request body to webhook!\n', req.body)
            ...
            if (req.body.type === 'event_callback') {
                ...
                res.sendStatus(200)
                if (req.body.event.user === req.body.authed_users[0]) {
                  slack_botkit.debug('Got event from this bot user, ignoring it')
                  return
                }
                // console.log('===============EVENT_CALLBACK received:\n', req.body)
                ...
                slack_botkit.findTeamById(message.team, function(err, team) {
                    if (err || !team) {
                        ...
                        bot.identifyBot(function(err, identity){
                          console.log('==========IDENTITY AFTER bot.identifyBot in callback:', identity)
                          bot.identity = identity

                          if (req.body.event.type === 'message') {
                              // console.log('***************** botkit.receieveMessage from webhook')
                              console.log('=============BOT IDENTITY RIGHT BEFORE receieveMessage in no team.bot.identity clause:', bot.identity)
                                slack_botkit.receiveMessage(bot, message);
                                // slack_botkit.trigger('message_received', [bot, message]);
                      ...
                      console.log('==========BOT IDENTITY AFTER bot.identifyBot:', bot.identity)


                              if (req.body.event.type === 'message') {
                                  // console.log('***************** botkit.receieveMessage from webhook')
                                  console.log('=============BOT IDENTITY RIGHT BEFORE receieveMessage:', bot.identity)
                                    slack_botkit.receiveMessage(bot, message);
                                    // slack_botkit.trigger('message_received', [bot, message]);
                            ...
                            slack_botkit.findTeamById(identity.team_id, function(err, team) {
                                ...
                                slack_botkit.saveTeam(team, function(err, id) {
                                    if (err) {
                                        ...
                                        if (!team.bot.idenity || team.bot.identity.id === null) {
                                          ...
                                          slack_botkit.findTeamById(identity.team_id, function(err, team_data) {
                                            if (!team_data.bot.identity) {
                                              ...
                                              auth_test({token: team_data.bot.token}, function(err, auth_data) {
                                                console.log('======AUTH DATA IN CALLBACK HELL:', auth_data)
                                                team_data.bot.identity = {
                                                  id: auth_data.user_id,
                                                  name: auth_data.user,
                                                  team_id: auth_data.team_id
                                                }
                                                slack_botkit.saveTeam(team_data, function(err, id) {
                                                  if (err) {
                                                    slack_botkit.log.error('An error occurred while saving a team: ', err);
                                                  }
    ...
    slack_botkit.handleSlackEvents = function() {
        ...
        slack_botkit.on('message_received', function(bot, message) {
            ...
            // console.log('=====Handle Slack Events Message Received message body:\n', message)
            ...
            if ('message' == message.type) {
                ...
                if (message.subtype && message.subtype == 'channel_join') {
                    ...
                    // slack_botkit.debug('********* Direct Message triggered!')
