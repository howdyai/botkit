    ...
    storageMethod.save(testObj0, function(err) {
        ...
        storageMethod.save(testObj1, function(err) {
            ...
            storageMethod.get('shouldnt-be-here', function(err, data) {
                //handle JFS get issue for in-memory returning empty err object
                test.assert(!err || Object.keys(err).length === 0);
                test.assert(!data);
            });
...
// Test simple_storage in memory
var simple_storage_memory = require('./simple_storage.js')({
    memory: true
});
testStorageMethod(simple_storage_memory.users);
testStorageMethod(simple_storage_memory.channels);
testStorageMethod(simple_storage_memory.teams);
