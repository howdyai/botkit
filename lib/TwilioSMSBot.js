...
function TwilioSMS(configuration) {
    ...
    if (!configuration) {
        throw Error(
            'Specify your \'account_sid\', \'auth_token\', and ' +
    ...
    twilioSMS.defineBot(function(botkit, config) {
        ...
        bot.send = function(sms, cb) {
            var client = new Twilio(configuration.account_sid, configuration.auth_token);
    ...
    twilioSMS.createWebhookEndpoints = function(webserver, bot, cb) {
        twilioSMS.log(
            '** Serving webhook endpoints for Twilio Programmable SMS' +
                ' at: ' +
                os.hostname() +
                ':' +
                twilioSMS.config.port +
                '/sms/receive'
        );
    ...
    function verifyRequest(req, res, next) {
        ...
        var validation_url =
            twilioSMS.config.validation_url ||
