...
module.exports = function(bot, config) {
    ...
    var slack_api = {
        ...
        callAPI: function(command, options, cb) {
            ...
            bot.middleware.pre_api.run(command, options, function(err, command, options) {
                request.post(slack_api.api_url + command, function(error, response, body) {
                    bot.debug('Got response', error, body);
                    if (!error && response.statusCode == 200) {
                        var json = JSON.parse(body);
                        if (json.ok) {
                            bot.middleware.post_api.run(command, options, json, function(err, command, options, json) {
                                if (cb) cb(null, json);
                            });
                        } else {
