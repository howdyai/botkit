...
var debug = require('debug')('slack:api:debug');
var log = require('debug')('slack:api:log');

module.exports = function(bot, config) {
    debug.log = function() {
        if (bot.logger && bot.logger.log)
            bot.logger.log.apply(bot.logger, ['info'].concat(arguments));
    };
    log.log = debug.log;
    ...
    var slack_api = {
        ...
        callAPI: function(command, options, cb) {
            log('Request API call with token at', slack_api.api_url + command);
            if (!options.token) {
        ...
        callAPIWithoutToken: function(command, options, cb) {
            log('Request API call without token at', slack_api.api_url + command);
            ...
            request.post(this.api_url + command, function(error, response, body) {
                ...
                if (!error && response.statusCode == 200) {
                    ...
                    if (json.ok) {
                        ...
                        cb && cb(json.error, json);
                    }
                } else {
                    cb && cb(error || 'Invalid response');
                }
            }).form(options);
        },
        auth: {
