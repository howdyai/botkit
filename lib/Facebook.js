...
function Facebookbot(configuration) {
    ...
    var api_version = configuration.fb_api_version || 'v2.11';
    ...
    facebook_botkit.defineBot(function (botkit, config = {}) {
        ...
        bot.send = function (message, cb) {
            ...
            //Add Access Token to outgoing request - allow multi-tenant support with access_token defined at runtime
            message.access_token = bot.config.access_token || configuration.access_token;

            if (bot.config.require_appsecret_proof) {
    ...
    var messenger_profile_api = (configuration) => ({
    ...
    var attachment_upload_api = configuration => ({
        upload: function (attachment, cb) {
            var uri = 'https://' + api_host + '/' + configuration.api_version + '/me/message_attachments?access_token=' + configuration.access_token;
    ...
    var personas_api = {
        ...
        postAPI: function (persona) {
            return new Promise(function (resolve, reject) {
                ...
                if (facebook_botkit.config.require_appsecret_proof) {
                    uri += '&appsecret_proof=' + getAppSecretProof(configuration.access_token, configuration.app_secret);
                }
        ...
        getAPI: function (persona_id) {
            return new Promise(function (resolve, reject) {
                ...
                if (configuration.require_appsecret_proof) {
                    uri += '&appsecret_proof=' + getAppSecretProof(configuration.access_token, configuration.app_secret);
                }
    ...
    function scopeAPIsWithRuntimeConfiguration(configuration) {
        return {
            'user_profile': user_profile(configuration),
            'messenger_profile': messenger_profile_api(configuration),
            'thread_settings': messenger_profile_api(configuration),
            'attachment_upload': attachment_upload_api(configuration),
            'nlp': nlp(configuration),
            'tags': tags(configuration),
            'handover': handover(configuration),
            'broadcast': broadcast_api(configuration),
            'insights': insights_api(configuration),
            'personas': personas_api(configuration),
        };
    }
