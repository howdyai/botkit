...
function Facebookbot(configuration) {
    ...
    facebook_botkit.getInstanceInfo = function(cb) {
        return new Promise(function(resolve, reject) {
            ...
            request.get('https://' + api_host + '/' + api_version+ '/me?access_token=' + configuration.access_token,
                {},
                function(err, res, body) {
                    if (err) {
                        if (cb) cb(err);
    ...
    var messenger_profile_api = {
        ...
        postAPI: function(message) {
            request.post('https://' + api_host + '/' + api_version + '/me/messenger_profile?access_token=' + configuration.access_token,
                {form: message},
                function(err, res, body) {
                    if (err) {
        ...
        deleteAPI: function(type) {
            ...
            request.delete('https://' + api_host + '/' + api_version + '/me/messenger_profile?access_token=' + configuration.access_token,
                {form: message},
                function(err, res, body) {
                    if (err) {
                        facebook_botkit.log('Could not configure messenger profile');
        ...
        getAPI: function(fields, cb) {
            request.get('https://' + api_host + '/' + api_version + '/me/messenger_profile?fields=' + fields + '&access_token=' + configuration.access_token,
                function(err, res, body) {
                    if (err) {
        ...
        get_messenger_code: function(image_size, cb, ref) {
            ...
            request.post('https://' + api_host + '/' + api_version + '/me/messenger_codes?access_token=' + configuration.access_token,
    ...
    var attachment_upload_api = {
        upload: function(attachment, cb) {
            ...
            request.post('https://' + api_host + '/' + api_version + '/me/message_attachments?access_token=' + configuration.access_token,
                { form: message },
                function(err, res, body) {
                    if (err) {
    ...
    var tags = {
        get_all: function(cb) {
            request.get('https://' + api_host + '/' + api_version + '/page_message_tags?access_token=' + configuration.access_token,
                function(err, res, body) {
                    if (err) {
    ...
    var broadcast_api = {
        ...
        send: function(message_creative_id, cb) {
            ...
            request({
                ...
                if (err) {
                    ...
                    if (body.error) {
                        facebook_botkit.log('ERROR in sending a broadcast message call : ', body.error);
                        cb(body.error);
                    } else {
                        facebook_botkit.debug('Successfully send a broadcast message', body);
                        cb(null, body);
                    }
                }
            });
            ...
            request.get(uri, {}, function(err, res, body) {
                if (err) {
                    ...
                    try {
                        ...
                        facebook_botkit.log('ERROR in getting broadcast metrics call: Could not parse JSON', err, body);
                        cb(err);
                    }

                    if (results) {
                        if (results.error) {
                            facebook_botkit.log('ERROR in getting broadcast metrics call : ', results.error.message);
                            cb(results.error);
                        } else {
        ...
        create_label: function(name, cb) {
            ...
            request({
                ...
                if (err) {
                    facebook_botkit.log('Could not create a label to target broadcast messages');
                    cb(err);
                } else {
                    if (body.error) {
                        facebook_botkit.log('ERROR in creating a label to target broadcast messages call : ', body.error);
                        cb(body.error);
                    } else {
                        facebook_botkit.debug('Successfully create a label to target broadcast messages', body);
                        cb(null, body);
                    }
                }
            });
        ...
        get_labels_by_user: function(user, cb) {
            ...
            request({
                ...
                if (err) {
                    facebook_botkit.log('Could not get labels associated with a user');
                    cb(err);
                } else {
                    if (body.error) {
                        facebook_botkit.log('ERROR in getting labels associated with a user call : ', body.error.message);
                        cb(body.error);
                    } else {
                        facebook_botkit.log('Successfully get labels associated with a user');
                        cb(null, body);
                    }
                }
            });
