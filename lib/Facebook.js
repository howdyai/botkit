...
function Facebookbot(configuration) {
    ...
    var build_in_nlp = {
        enable: function() {
            facebook_botkit.api.build_in_nlp.postAPI(true);
        },
        disable: function() {
            facebook_botkit.api.build_in_nlp.postAPI(false);
        },
        postAPI: function(value) {
            request.post('https://' + api_host + '/v2.8/me/nlp_configs?nlp_enabled=' + value + '&access_token=' + configuration.access_token,
                {},
                function(err, res, body) {
                    if (err) {
