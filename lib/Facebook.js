...
function Facebookbot(configuration) {
    ...
    facebook_botkit.defineBot(function(botkit, config) {
		...
		bot.broadcast = (message, label, cb) => {
			return bot.controller.api.broadcast.send(message, label, cb)
		}
	...
	const _api = {
		post: function postAPI(url, body, cb) {
		...
		delete: function deleteAPI(url, body, cb) {
			return new Promise((resolve, reject) => {
				rp({
					url,
					qs: {
	...
	const createBroadcastLabel = function(name, cb) {
		const url = 'https://graph.facebook.com/v2.11/me/custom_labels'
		return _api.post(url, {name}, cb)
	}

	const addUserToLabel = function(user, label, cb) {
		const url = `https://graph.facebook.com/v2.11/${label}/label`
		return _api.post(url, {user}, cb)
	}

	const removeUserFromLabel = function(user, label, cb) {
		const url = `https://graph.facebook.com/v2.11/${label}/label`
		console.log('USER IN DELETE!!', user)
		return _api.delete(url, {user}, cb)
	}

	const getUserLabels = function(user, cb) {
		const url = `https://graph.facebook.com/v2.11/${user}/custom_labels?fields=name`
		return _api.get(url, cb)
	}
	
	const getLabelInfo = function(label, cb) {
		const url = `https://graph.facebook.com/v2.11/${label}?fields=name`
		return _api.get(url, cb)
	}

	const getAllLabels = function(cb) {
		const url = `https://graph.facebook.com/v2.11/me/custom_labels?fields=name`
		return _api.get(url, cb)
	}

	const deleteLabel = function(label, cb) {
		const url = `https://graph.facebook.com/v2.11/${label}`
		return _api.delete(url, cb)
	}

	const createBroadcast = function(messages, cb) {
		if (! Array.isArray(messages)) {
			messages = [messages]
		}
		const url =  'https://graph.facebook.com/v2.11/me/message_creatives'
		return _api.post(url, {messages}, cb)
	}

	const sendBroadcast = function(message, label, cb) {
		const url = 'https://graph.facebook.com/v2.11/me/broadcast_messages'
		if (typeof label === 'function' && !cb) {
			cb = label
			label = null
		}
		...
		if (typeof message === 'string') {
			...
			return _api.post(url, body, cb)

		} else {
			return createBroadcast(message, cb)
				.then(res => {
					const body = {
					...
					return _api.post(url, body, cb)
					.catch(err => facebook_botkit.debug('Error sending broadcast:', err.message))
				}).catch(err => {
					facebook_botkit.debug('Error creating broadcast:', err)
				})
		}
	}
	...
	var broadcast = {
		labels: {
			create: createBroadcastLabel,
			addUser: addUserToLabel,
			removeUser: removeUserFromLabel,
			getUser: getUserLabels,
			getLabel: getLabelInfo,
			getAll: getAllLabels,
			delete: deleteLabel
		},
		send: sendBroadcast,
