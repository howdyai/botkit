...
function Facebookbot(configuration) {
    ...
    facebook_botkit.setupWebserver = function(port, cb) {
        ...
        var access_tokens = (typeof (configuration.access_token) === 'string') ? { 0: configuration.access_token } : configuration.access_token;
    ...
    var messenger_profile_api = {
        ...
        postAPI: function (message) {
            var access_tokens = (typeof (configuration.access_token) === 'string') ? { 0: configuration.access_token } : configuration.access_token;
            Object.keys(access_tokens).forEach(function(page) {
                request.post('https://' + api_host + '/v2.6/me/messenger_profile?access_token=' + access_tokens[page],
                    {form: message},
                    function(err, res, body) {
                        if (err) {
        ...
        deleteAPI: function(type) {
            ...
            var access_tokens = (typeof (configuration.access_token) === 'string') ? { 0: configuration.access_token } : configuration.access_token;
            Object.keys(access_tokens).forEach(function(page) {
                request.delete('https://' + api_host + '/v2.6/me/messenger_profile?access_token=' + access_tokens[page],
                    {form: message},
                    function(err, res, body) {
                        if (err) {
                            facebook_botkit.log('Could not configure messenger profile with access_token', access_tokens[page]);
        ...
        getAPI: function(fields, cb) {
            var access_tokens = (typeof (configuration.access_token) === 'string') ? { 0: configuration.access_token } : configuration.access_token;
            Object.keys(access_tokens).forEach(function(page) {
                request.get('https://' + api_host + '/v2.6/me/messenger_profile?fields=' + fields + '&access_token=' + access_tokens[page],
                    function(err, res, body) {
                        if (err) {
        ...
        get_messenger_code: function(image_size, cb, ref) {
            ...
            var access_tokens = (typeof (configuration.access_token) === 'string') ? { 0: configuration.access_token } : configuration.access_token;
            Object.keys(access_tokens).forEach(function(page) {
                request.post('https://' + api_host + '/v2.6/me/messenger_codes?access_token=' + access_tokens[page],
                    {form: message},
                    function(err, res, body) {
                        if (err) {
