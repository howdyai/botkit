...
function Facebookbot(configuration) {
    ...
    facebook_botkit.defineBot(function(botkit, config) {
        ...
        bot.replyWithTyping = function(src, resp, cb) {
            ...
            if (typeof(resp) == 'string') {
                ...
                textLength = 30; //default attachement text length
            }

            var avgWPM = 200;
            var avgCPM = avgWPM * 7;

            var typingLength = Math.min(Math.floor(textLength / (avgCPM / 60)) * 1000, 2000);
	...
	facebook_botkit.setupWebserver = function(port, certs_or_cb, cb) {

        var certs = null;
        //certs object example
        /*  {
        ...
        */
        ...
        if(certs){
            var server = https.createServer(certs,facebook_botkit.webserver).listen(
