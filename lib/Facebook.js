...
function Facebookbot(configuration) {
    ...
    facebook_botkit.defineBot(function(botkit, config) {
        ...
        bot.findConversation = function(message, cb) {
            botkit.debug('**Bot :',bot.identity.name, 'CUSTOM FIND CONVO', message.user, message.channel);
            for (var t = 0; t < botkit.tasks.length; t++) {
                for (var c = 0; c < botkit.tasks[t].convos.length; c++) {
                    if (
                        ...
                        botkit.debug('**Bot :',bot.identity.name, '  FOUND EXISTING CONVO!');
    ...
    facebook_botkit.createWebhookEndpoints = function(webserver, bot, cb) {
        ...
        webserver.post('/facebook/receive', function(req, res) {
            ...
            facebook_botkit.debug('**Bot :',bot.identity.name, '  GOT A MESSAGE HOOK');
            var obj = req.body;
            if (obj.entry) {
        ...
        request.post('https://graph.facebook.com/me/subscribed_apps?access_token=' + configuration.access_token,
            function(err, res, body) {
                if (err) {
                    ...
                    facebook_botkit.debug('**Bot :',bot.identity.name, '  Successfully subscribed to Facebook events:', body);
