...
function Facebookbot(configuration) {
    ...
    if (!configuration.access_token && !configuration.access_token_cb) {
        throw new Error('One of access_token or access_token_cb must be set.');
    }
    ...
    facebook_botkit.defineBot(function(botkit, config) {
        ...
        bot.send = function(message, cb) {
            var access_token = typeof(facebook_botkit.config.access_token) !== 'undefined' ? facebook_botkit.config.access_token : facebook_botkit.config.access_token_cb(message.page_id);
            ...
            facebook_message.access_token = access_token;
                    ...
                    if (facebook_message.message) {
                        ...
                        if (facebook_botkit.config.receive_via_postback) {
                            var message = {
                                ...
                                referral: facebook_message.postback.referral,
                                page_id: facebook_message.postback.page_id,
                            };
    ...
    facebook_botkit.setupWebserver = function(port, cb) {
        ...
        // Backwards compatibility - automatically subscribe app if a single access token was configured.
        if (facebook_botkit.config.access_token) {
          subscribeToWebhook(facebook_botkit.config.access_token);
        }
    ...
    var messenger_profile_api = {
        greeting: function(pageId, payload) {
            var message = {
                'greeting': []
            };
            ...
            facebook_botkit.api.messenger_profile.postAPI(message, pageId);
        },
        delete_greeting: function(pageId) {
            facebook_botkit.api.messenger_profile.deleteAPI('greeting', pageId);
        },
        get_greeting: function(pageId, cb) {
            facebook_botkit.api.messenger_profile.getAPI('greeting', pageId, cb);
        },
        get_started: function(pageId, payload) {
            var message = {
                'get_started': {
                    'payload': payload
                }
        ...
        delete_get_started: function(pageId) {
            facebook_botkit.api.messenger_profile.deleteAPI('get_started', pageId);
        },
        get_get_started: function(pageId, cb) {
            facebook_botkit.api.messenger_profile.getAPI('get_started', pageId, cb);
        },
        menu: function(pageId, payload) {
            var message = {
                persistent_menu: payload
            };
            facebook_botkit.api.messenger_profile.postAPI(message, pageId);
        },
        delete_menu: function(pageId) {
            facebook_botkit.api.messenger_profile.deleteAPI('persistent_menu', pageId);
        },
        get_menu: function(pageId, cb) {
            facebook_botkit.api.messenger_profile.getAPI('persistent_menu', pageId, cb);
        },
        account_linking: function(pageId, payload) {
            var message = {
                'account_linking_url': payload
            };
            facebook_botkit.api.messenger_profile.postAPI(message, pageId);
        },
        delete_account_linking: function(pageId) {
            facebook_botkit.api.messenger_profile.deleteAPI('account_linking_url', pageId);
        },
        get_account_linking: function(pageId, cb) {
            facebook_botkit.api.messenger_profile.getAPI('account_linking_url', pageId, cb);
        },
        domain_whitelist: function(pageId, payload) {
            var message = {
                'whitelisted_domains': Array.isArray(payload) ? payload : [payload]
            };
            facebook_botkit.api.messenger_profile.postAPI(message, pageId);
        },
        delete_domain_whitelist: function(pageId) {
            facebook_botkit.api.messenger_profile.deleteAPI('whitelisted_domains', pageId);
        },
        get_domain_whitelist: function(pageId, cb) {
            facebook_botkit.api.messenger_profile.getAPI('whitelisted_domains', pageId, cb);
        },
        postAPI: function(message, pageId) {
            var access_token = typeof(configuration.access_token) !== 'undefined' ? configuration.access_token : configuration.access_token_cb(pageId);

            request.post('https://graph.facebook.com/v2.6/me/messenger_profile?access_token=' + access_token,
                {form: message},
                function(err, res, body) {
                    if (err) {
        ...
        deleteAPI: function(type, pageId) {
            var access_token = typeof(configuration.access_token) !== 'undefined' ? configuration.access_token : configuration.access_token_cb(pageId);

            var message = {
                'fields': [type]
            };
            request.delete('https://graph.facebook.com/v2.6/me/messenger_profile?access_token=' + access_token,
                {form: message},
                function(err, res, body) {
                    if (err) {
                        facebook_botkit.log('Could not configure messenger profile');
        ...
        getAPI: function(fields, pageId, cb) {
            var access_token = typeof(configuration.access_token) !== 'undefined' ? configuration.access_token : configuration.access_token_cb(pageId);

            request.get('https://graph.facebook.com/v2.6/me/messenger_profile?fields=' + fields + '&access_token=' + access_token,
                function(err, res, body) {
                    if (err) {
        ...
        get_messenger_code: function(image_size, pageId, cb) {
            var access_token = typeof(configuration.access_token) !== 'undefined' ? configuration.access_token : configuration.access_token_cb(pageId);
            ...
            request.post('https://graph.facebook.com/v2.6/me/messenger_codes?access_token=' + access_token,
                {form: message},
                function(err, res, body) {
                    if (err) {
    ...
    function subscribeToWebhook(access_token) {
        request.post('https://' + api_host + '/me/subscribed_apps?access_token=' + access_token,
            function(err, res, body) {
                if (err) {
