        ...
        if (obj.entry) {
            for (var e = 0; e < obj.entry.length; e++) {
                for (var m = 0; m < obj.entry[e].messaging.length; m++) {
                    ...
                    } else if (facebook_message.app_roles) {
                        var message = {
                            page: facebook_message.recipient.id,
                            timestamp: facebook_message.timestamp,
                            app_roles: facebook_message.app_roles
                        };

                        facebook_botkit.trigger('facebook_app_roles', [bot, message]);
                    } else if (facebook_message.object && facebook_message.object === "page") {

                        var standby_entry = facebook_message.entry.filter(function(entry) {
                            return entry.standby;
                        })[0];

                        if(standby_entry) {
                            var message = {
                                page: standby_entry.id,
                                timestamp: standby_entry.time,
                                standby: standby_entry.standby
                            };

                            facebook_botkit.trigger('facebook_standby', [bot, message]);
                        } else {
                            facebook_botkit.log('Got an unexpected page object entry from Facebook: ', facebook_message);
                        }
                    } else if (facebook_message.pass_thread_control) {
                        var message = {
                            user: facebook_message.sender.id,
                            page: facebook_message.recipient.id,
                            timestamp: facebook_message.timestamp,
                            pass_thread_control: facebook_message.pass_thread_control
                        };

                        facebook_botkit.trigger('facebook_pass_thread_control', [bot, message]);
                    } else if (facebook_message.take_thread_control) {
                        var message = {
                            user: facebook_message.sender.id,
                            page: facebook_message.recipient.id,
                            timestamp: facebook_message.timestamp,
                            take_thread_control: facebook_message.take_thread_control
                        };

                        facebook_botkit.trigger('facebook_take_thread_control', [bot, message]);
                    }  else {
                    ...
                    if (err) {
                        ...
                        if (results) {
                            if (results.error) {
                                ...
                                facebook_botkit.debug('Successfully take thread control', body);
                                cb(result);
                            }
                        }
                    }
