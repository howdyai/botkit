        ...
        if (obj.entry) {
            for (var e = 0; e < obj.entry.length; e++) {
                for (var m = 0; m < obj.entry[e].messaging.length; m++) {
                    ...
                    if (facebook_message.message && !facebook_message.message.quick_reply) {
                        var message = {
                    ...
                    } else if (facebook_message.message && facebook_message.message.quick_reply) {

                        // trigger BOTH a quick_reply event
                        // and a normal message received event.
                        // this allows developers to receive quick_reply as part of a conversation.
                        var message = {
                            text: facebook_message.message.quick_reply.payload,
                            payload: facebook_message.message.quick_reply.payload,
                            input: facebook_message.message.text,
                            user: facebook_message.sender.id,
                            channel: facebook_message.sender.id,
                            page: facebook_message.recipient.id,
                            timestamp: facebook_message.timestamp,
                        ...
                        facebook_botkit.trigger('facebook_quick_reply', [bot, message]);
