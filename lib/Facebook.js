    ...
    facebook_botkit.api = {
        'thread_settings': {
            ...
            get_domain_whitelist: function(cb) {
                request('https://graph.facebook.com/v2.6/me/thread_settings?fields=whitelisted_domains&access_token=' + configuration.access_token,
                    function(err, res, body) {
                        if (err) {
                            facebook_botkit.log('Could not get whitelisted domains');
                            cb(err);
                        } else {
                            var results = null;
                            try {
                                ...
                                facebook_botkit.log('ERROR in thread_settings API call: Could not get whitelisted domains JSON', err, body);
                                cb(err);
                            }
                            ...
                            if (results) {
                                if (results.error) {
                                    ...
                                    facebook_botkit.debug('Successfully getting whitelisted domains', body);
                                    cb(null, body);
                                }
                            }
