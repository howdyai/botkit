...
var debug = require('debug')('facebook:bot:debug');
var log = require('debug')('facebook:bot:log');
...
function Facebookbot(configuration) {
    ...
    facebook_botkit.defineBot(function(botkit, config) {
            ...
            request.post('https://graph.facebook.com/me/messages?access_token=' + configuration.access_token,
                function(err, res, body) {
                    ...
                    try {
                        ...
                        debug('Error occurred while parsing JSON', err);
                        return cb && cb(err);
                    }

                    if (json.error) {
                        debug('Error in Facebook API', json.error);
                        return cb && cb(json.error.message);
                    }

                    debug('Success on Facebook hook', body);
                    cb && cb(null, body);
        ...
        bot.findConversation = function(message, cb) {
            debug('Initialize find conversation with user', message.user, 'at', message.channel);
    ...
    facebook_botkit.createWebhookEndpoints = function(webserver, bot, cb) {
        ...
        webserver.post('/facebook/receive', function(req, res) {
            ...
            facebook_debug('Receive message hook');
            var obj = req.body;
            if (obj.entry) {
                for (var e = 0; e < obj.entry.length; e++) {
                    for (var m = 0; m < obj.entry[e].messaging.length; m++) {
                        ...
                        if (facebook_message.message) {
                            ...
                            log('Got an unexpected message from Facebook: ', facebook_message);
                        }
                    }
                }
            }
    ...
    facebook_botkit.setupWebserver = function(port, cb) {
        ...
        var server = facebook_botkit.webserver.listen(
            ...
            function() {
                facebook_log('** Starting webserver on port ' +
                    facebook_botkit.config.port);
        ...
        request.post('https://graph.facebook.com/me/subscribed_apps?access_token=' + configuration.access_token,
            function(err, res, body) {
                if (err) {
                    facebook_log('Failure to subscribe to events');
                } else {
                    facebook_debug('Success while subscribing to events', body);
