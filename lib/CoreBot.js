...
function Botkit(configuration) {
    ...
    botkit.ingest = function(obot, opayload, source) {
        ...
        botkit.middleware.ingest.run(obot, opayload, source, function(err, bot, payload, source) {
            if (err) {
                console.error('An error occured in the ingest middleware: ', err);
                botkit.trigger('ingest_error', [err, obot, omessage]);
                botkit.trigger('pipeline_error', [err, obot, omessage, 'ingest']);
                return;
            }
    ...
    botkit.normalize = function(obot, opayload) {
        ...
        botkit.middleware.normalize.run(obot, opayload, function(err, bot, message) {
            if (err) {
                ...
                botkit.trigger('normalize_error', [err, obot, omessage]);
                botkit.trigger('pipeline_error', [err, obot, omessage, 'normalize']);
                return;
            }
