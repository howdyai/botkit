...
function Botkit(configuration) {
    ...
    botkit.trigger = function(event, data) {
        ...
        var bot = data[0];
        var message = data[1];
        if ((botkit.listeningFor[event] && botkit.listeningFor[event].length) || (botkit.events[event] && botkit.events[event].length)) {
            botkit.middleware.hears.run(bot, message, botkit.listeningFor[event]||[], function(err, bot, message, triggers) {
