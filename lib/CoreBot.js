...
function Botkit(configuration) {
    ...
    function Conversation(task, message) {
        ...
        this.capture = function(response, cb) {
            ...
            botkit.middleware.capture.run(that.task.bot, response, that, function(
                err,
                bot,
                response,
                convo
                ...
                this.capture(message, function(message) {
                    ...
                    if (typeof that.handler == 'function') {
                        ...
                        for (var p = 0; p < patterns.length; p++) {
                            if (
                                patterns[p].pattern &&
                                botkit.hears_test([patterns[p].pattern], message)
                            ) {
                                botkit.middleware.heard.run(that.task.bot, message, function(
                                    err,
                                    bot,
                                    message
                                ) {
                        ...
                        if (condition.execute) {
                            ...
                            botkit.studio
                                ...
                                .then(function(new_convo) {
                                    that.context.transition_to =
                                        new_convo.context.script_name || null;
                                    that.context.transition_to_id =
                                    ...
                                    new_convo.context.transition_from =
                                        that.context.script_name || null;
                                    new_convo.context.transition_from_id =
                                        that.context.script_id || null;
        ...
        this.cloneMessage = function(message) {
            ...
            if (outbound.attachment) {
                ...
                if (
                    ...
                    typeof outbound.attachment.payload.text != 'string'
                ) {
                        ...
                        if (
                            ...
                            if (message.conditional) {
                                ...
                                if (message.text || message.attachments || message.attachment) {
                                    ...
                                    this.task.bot.reply(this.source_message, outbound, function(
                                        err,
                                        sent_message
                                    ) {
                                        if (err) {
    ...
    function Task(bot, message, botkit) {
        ...
        this.taskEnded = function() {
            botkit.log(
                ...
                this.id,
                ' Task for ',
                this.source_message.user,
                'in',
                this.source_message.channel
            );
