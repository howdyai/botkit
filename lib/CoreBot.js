          ...
          switch(conditional.action) {
                case 'execute_script':
                    if (conditional.execute) {
                        var script = conditional.execute.script;
                        var thread = conditional.execute.thread;
                        controller.studio.get(this.context.bot, script, this.source_message.user, this.source_message.channel, this.source_message).then(function(new_convo) {
                            convo.stop('transitioning to ', script);
                            new_convo.responses = this.responses;
                            new_convo.vars = this.vars;
