...
var debug = require('debug')('core:bot:debug');
var log = require('debug')('core:bot:log');
...
function Botkit(configuration) {
    ...
    function Conversation(task, message) {
        ...
        this.on = function(event, cb) {
            debug('Initialize event handler', event);
        ...
        this.trigger = function(event, data) {
            if (this.events[event]) {
                ...
                debug('Failed to trigger event', event);
            }
        };
        ...
        this.replaceTokens = function(text) {
            ...
            try {
                ...
                log('Failure in Mustache render', err);
                rendered = text;
            };
                        ...
                        if (typeof(this.messages[0].timestamp) == 'undefined' ||
                            ...
                            if (message.text || message.attachments || message.attachment) {
                                ...
                                this.task.bot.reply(this.source_message, outbound, function(err) {
                                    if (err) {
                                        log('Failure in sending message', err);
                                    }
                                });
                            }
                            if (message.action) {
        ...
        debug('Conversation created for', this.source_message.user, 'at channel', this.source_message.channel);
    ...
    function Task(bot, message, botkit) {
        ...
        this.startConversation = function(message) {
            ...
            log('Start conversation with id', convo.id, 'with', message.user, 'in', message.channel);
        ...
        this.conversationEnded = function(convo) {
            log('End conversation with id', convo.id, 'with',
        ...
        this.taskEnded = function() {
            log('End task with id', this.id, 'for',
    ...
    botkit.storage = {
        teams: {
            ...
            save: function(team, cb) {
                log('WARNING: Temporary storage detected. Data will be lost when process restarts.');
                if (team.id) {
            ...
            (function(keywords, test_function) {
                botkit.on(events[e], function(bot, message) {
                    if (test_function(keywords, message)) {
                        debug('Trigger heard for keywords', keywords);
    ...
    botkit.trigger = function(event, data) {
        if (this.events[event]) {
            ...
            debug('Failure to find event handler for', event);
        }
    };
    ...
    botkit.startTask = function(bot, message, cb) {
        ...
        log('Start task with id', task.id, 'for', message.user, 'in', message.channel);
    ...
    botkit.receiveMessage = function(bot, message) {
        botkit.middleware.receive.run(bot, message, function(err, bot, message) {
            if (err) {
                log('Failure to receive message', err);
            } else {
    ...
    botkit.worker = function(botkit, config) {
        ...
        this.reply = function(src, resp) {
            debug('Trigger `reply`', resp);
        };


        this.findConversation = function(message, cb) {
            debug('Trigger default find conversation');
            cb(null);
        };
    };
    ...
    if (configuration.storage) {
        if (
            ...
            log('Initialize custom storage system.');
        ...
        log('Use simple storage. Saving data to', configuration.json_file_store);
        botkit.storage = simple_storage({path: configuration.json_file_store});
    } else {
        log('WARNING: No persistent storage method specified! Data may be lost when process shuts down.');
    }
