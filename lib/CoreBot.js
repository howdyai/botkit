                    ...
                    if (this.messages.length) {
                        if (typeof(this.messages[0].timestamp) == 'undefined' ||
                            ...
                            if (message.text || message.attachments || message.attachment) {
                                ...
                                var bot = this.task.bot;
                                var source_message = this.source_message;
                                var delay = message.delay || 500;

                                botkit.log("Sending message with delay : %s", delay);

                                setTimeout(function() {

                                    bot.reply(source_message, outbound, function(err) {
                                        if (err) {
