...
// var _BOTKIT_COMMAND_API = 'http://192.168.33.19';
...
module.exports = function(botkit) {
    ...
    botkit.compileScript = function(bot, message, command_name, topics, vars) {
        function makeHandler(options, field) {
            ...
            if (options.type == 'utterance') {
                pattern = botkit.utterances[options.pattern];
            } else {
            ...
            return {
                ...
                callback: function(response, convo) {
                    ...
                    // if (options.default) {
                        ...
                        // if we're moving on to next,
                        // and multiple lines are enabled
                        // get rid of the most recent response
        ...
        return new Promise(function(resolve,reject) {
            bot.createConversation(message, function(err, convo) {
                ...
                convo.setTimeout(botkit.config.default_timeout || (15 * 60 * 1000));  // 15 minute default timeout
    ...
    botkit.runRemote = function(bot, message) {
        return new Promise(function(resolve,reject) {
            botkit.getCommandByName(bot, message.text).then(function(command) {
