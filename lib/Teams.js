...
function TeamsBot(configuration) {
  ...
  controller.defineBot(function(botkit, config) {
    ...
    bot.reply = function(src, resp, cb) {
      ...
      bot.send(controller.slackToTeams(resp), cb);
    };
    ...
    if (message.attachments) {
      for (var a = 0; a < message.attachments.length; a++) {
        ...
        if (slack.title) {
          if (slack.title_link) {
            text =
              text + '# [' + slack.title + '](' + slack.title_link + ')\n\n';
          } else {
        ...
        if (slack.actions) {
          for (var b = 0; b < slack.actions.length; b++) {
            ...
            button.value =
              'button:' + slack.actions[b].name + ':' + slack.actions[b].value;
      ...
      console.log('TEAMS ATTACHMENT: ', message);
    }
