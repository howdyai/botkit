...
function Sparkbot(configuration) {
        ...
        var list = controller.api.webhooks.list().then(function(list) {
            ...
            if (hook_id) {
                controller.api.webhooks.update({
                    ...
                    console.log('Spark: SUCCESSFULLY UPDATED CISCO SPARK WEBHOOKS');
                    if (cb) cb();
                }).catch(function(err) {
    ...
    controller.defineBot(function(botkit, config) {
        ...
        bot.startPrivateConversation = function(message, cb) {
            ...
            botkit.startTask(bot, {
                toPersonEmail: message.user
            }, function(task, convo) {
                convo.on('sent', function(sent_message) {
                    ...
                    convo.source_message.user = sent_message.personEmail;
