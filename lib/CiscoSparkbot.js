...
function Sparkbot(configuration) {
    ...
    // ensure that the encrypted signature from Cisco Spark matches our configured secret
    // this will validate that the incoming webhook really came from Cisco.
    controller.middleware.ingest.use(function validateSecret(bot, message, res, next) {

        if (controller.config.secret) {
            var signature = req.headers['x-spark-signature'];
            var hash = crypto.createHmac('sha1', controller.config.secret).update(JSON.stringify(message)).digest('hex');
