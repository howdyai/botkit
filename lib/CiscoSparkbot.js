...
function Sparkbot(configuration) {
    ...
    if (!controller.config.public_address) {
        ...
        if (!endpoint.hostname) {
            ...
            controller.config.public_address =
                endpoint.hostname + (endpoint.port ? ':' + endpoint.port : '');
        }
    }
    ...
    controller.createWebhookEndpoints = function(webserver, bot, cb) {
        ...
        var list = controller.api.webhooks
            .list()
            .then(function(list) {
                var hook_id = null;
    ...
    controller.middleware.ingest.use(function limitUsers(bot, message, res, next) {
        if (controller.config.limit_to_org) {
            ...
            if (
                ...
                message.raw_message.orgId != controller.config.limit_to_org
            ) {
                // this message is from a user outside of the proscribed org
                console.log(
                    'WARNING: this message is from a user outside of the proscribed org',
        ...
        if (controller.config.limit_to_domain) {
            ...
            if (!allowed) {
                console.log(
                    'WARNING: this message came from a domain that is outside of the allowed list',
                    controller.config.limit_to_domain
                );
