...
var ChatInjection = require(__dirname + '../controllers/chatInjection.js');
var builder = require('botbuilder');
var express = require('express');
var bodyParser = require('body-parser');
var base64 = require('base-64');
var utf8 = requeire('utf8');

function BotFrameworkBot(configuration) {

    var getTypingTimeout = function(textLength, wpm) {
        var avgWPM = wpm || 85;
        var avgCPM = avgWPM * 7;

        return Math.min(Math.floor(textLength / (avgCPM / 60)) * 1000, 5000);
    };
    ...
    bf_botkit.defineBot(function(botkit, config) {
        ...
        bot.replyWithTyping = function(src, resp, cb) {
            ...
            var typingMsg = Object.assign({}, msg, { 
                custom_typing: true, 
                text: '', 
                attachments: []
            }); 
            ...
            var sendMessage = function(err) {
                ...
                setTimeout(function() {
                    bot.say(msg, cb);
                }, getTypingTimeout(textLength, bot.botkit.config.wpm));
            };

            bot.say(typingMsg, sendMessage);
        };
    ...
    // Set up a web route to inject frontend code into the client's website.

    bf_botkit.createInjectionEndpoint = function(webserver, cb) {
        webserver.get('/botframework/chat-injection', function (req, res) {
            ...
            var orgin = req.get('orgin');
            var ip = req.get('x-forwarded-for');

            console.log('=========== New request ==========');
            console.log(host, orgin, ip);
            console.log(req.params);

            var bytes = utf8.encode('pmnerds-phase2');
            ...
            if (decode in Object.keys(validation)) {
                res.sendStatus(200);
            } else {
                res.sendStatus(403);
            }
