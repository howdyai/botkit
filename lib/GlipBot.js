...
function Glipbot(configuration) {
    ...
    glip_botkit.createWebhookEndpoints = function(webserver, bot, cb){
        ...
        glip_botkit.log('** Serving Webhooks endpoint for receiving messages ** ' +
            'webhooks at: https://' + glip_botkit.config.apiRoot + ':' + glip_botkit.config.port + '/glip/receive');
    ...
    glip_botkit.configureGlipApp = function(glip_app_config, cb){
        ...
        glip_botkit.log('** Configuring app as a Glip App!');
    ...
    glip_botkit.createOauthEndpoints = function (webserver, bot, callback) {
        ...
        webserver.get('/oauth', function (req, res) {
            var apiKey = btoa(glip_botkit.config.clientId + ':' + glip_botkit.config.clientSecret);
            if(!req.query.code){
                res.status(500);
